...
(defun calculate-rating (l type-of-rating)
  ...
  (let ((filter-list l))
    (loop 
      for n from 0 upto (- (length filter-list) 1)
      until (= 1 (length filter-list)) 
      do
      (cond ((equal "o2" type-of-rating)
	     (setf filter-list 
		   (remove-if-not (lambda(x) (equal (get-popular-at-n filter-list n #\1) (aref x n))) filter-list)))
	    ((equal "co2" type-of-rating)
	     (setf filter-list 
...
; If I want to code up converting and multiplying the binary strings, this provides an option. 
